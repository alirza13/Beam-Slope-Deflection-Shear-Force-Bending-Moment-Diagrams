function plotDeflection(beamType, beamLength, beamWidth, beamHeight, beamMaterialType,elasticMod,inertia, pointForce, udl, windSpeed, fluidType)
    densityOfAir = 1.225;
    densityOfWater = 997;
    densityOfHoney = 1400;
    ...
    deflectionFormula = (-uniformLoad * x.^2) .* (6 * beamLength^2 - 4*beamLength*x + x.^2) / (24 * modElasticityChosen * secondMomentArea) + (-pointForce * x.^2) .* (3*beamLength - x) / (6 * modElasticityChosen * secondMomentArea);
    
    figure;
    hold on
%     sgtitle('10N Point Load, 50 N/m, Immersed in Honey')
    plot(x,deflectionFormula);
