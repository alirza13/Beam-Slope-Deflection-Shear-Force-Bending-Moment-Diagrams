...
global UDL WindSpeed FluidType
...
S.pb(2) = uicontrol('style','push',...
                    ...
                    'string','Width, Height, Beam Type, Elastic Modules, Inertia',...
...
function pb_wihetypeConfig(varargin)
    ...
    T.beamTypepb = uicontrol('style','push',...
                      'unit','pix',...
                      'position',[6 215 195 30],...
                      'string','Set beam material type');
    T.beamTypeed1 = uicontrol('style','edit',...
                         'unit','pix',...
                         'position',[6 250 131 30],...
                         'string','Beam Material:');
    T.beamTypeed2 = uicontrol('style','edit',...
                         'unit','pix',...
                         'position',[141 250 99 30],...
    ...
    T.elasticMopb = uicontrol('style','push',...
                      'unit','pix',...
                      'position',[6 145 195 30],...
                      'string','Set Elastic Modulus');
    T.elasticMoed1 = uicontrol('style','edit',...
                         'unit','pix',...
                         'position',[6 180 121 30],...
                         'string','Elastic Modulus [N/m^2]:');
    T.elasticMoed2 = uicontrol('style','edit',...
                         'unit','pix',...
                         'position',[131 180 99 30],...
                         'string','Positive number');
                 
    % Make sure the edit box cannot be altered.                 
    set(T.elasticMoed1,'enable','off');
    % Set callback when the button is pushed.
    set(T.elasticMopb,'call',{@elasticMo_callback,T});
    
    %INERTIA
    T.inertiapb = uicontrol('style','push',...
                      'unit','pix',...
                      'position',[6 75 195 30],...
                      'string','Set Inertia');
    T.inertiaed1 = uicontrol('style','edit',...
                         'unit','pix',...
                         'position',[6 110 121 30],...
                         'string','Inertia [m^4]:');
    T.inertiaed2 = uicontrol('style','edit',...
                         'unit','pix',...
                         'position',[131 110 99 30],...
                         'string','Positive number');
                 
    % Make sure the edit box cannot be altered.                 
    set(T.inertiaed1,'enable','off');
    % Set callback when the button is pushed.
    set(T.inertiapb,'call',{@inertia_callback,T});
...
function elasticMo_callback(varargin)
    global ElasticMod
    % Get structure from the main configuration setup function.
    T = varargin{3};

    % Import beam width string from the edit box. Then change it to number.
    elasticMod = get(T.elasticMoed2, 'String');
    elasticMod = str2double(elasticMod);
    if isnumeric(elasticMod)
        if elasticMod > 0
            ElasticMod = elasticMod;
            fprintf('Elastic Mod set to: %d [N/m^2].\n',ElasticMod);
        else
            fprintf('Elastic Mod must be a positive number.\n');
        end
    else
        fprintf('Elastic Mod must be a positive number.\n');
    end
end
function inertia_callback(varargin)
    global Inertia
    % Get structure from the main configuration setup function.
    T = varargin{3};

    % Import beam width string from the edit box. Then change it to number.
    inertia = get(T.inertiaed2, 'String');
    inertia = str2double(inertia);
    if isnumeric(inertia)
        if inertia > 0
            Inertia = inertia;
            fprintf('Inertia set to: %d [m^4].\n',Inertia);
        else
            fprintf('Inertia must be a positive number.\n');
        end
    else
        fprintf('Inertia must be a positive number.\n');
    end
...
function pb_uniLoadConfig (varargin)
    ...
    % Create new figure
    T.fh = figure('units','pixels',...
                  'position',[440 210 220 170],...
                  'menubar','none',...
                  'numbertitle','off',...
                  'name','UDL configuration',...
                  'resize','off');
              
        %Create width setup
    T.udlpb = uicontrol('style','push',...
                      'unit','pix',...
                      'position',[6 95 195 30],...
                      'string','Set uniform load');
    T.udled1 = uicontrol('style','edit',...
                         'unit','pix',...
                         'position',[6 140 101 30],...
                         'string','Uniform Load [N/m]:');
    T.udled2 = uicontrol('style','edit',...
                         'unit','pix',...
                         'position',[111 140 99 30],...
                         'string','Positive number');
                 
    % Make sure the edit box cannot be altered.                 
    set(T.udled1,'enable','off');
    % Set callback when the button is pushed.
    set(T.udlpb,'call',{@udl_callback,T});
    
    T.exit = uicontrol('style','push',...
                       'units','pix',...
                       'position',[10 10 183 50],...
                       'string','Return to main menu',...
                       'callback',{@pb_close,T});  
...
function udl_callback(varargin)
    % Load beam width.
global UDL
...
% Import beam width string from the edit box. Then change it to number.
udl = get(T.udled2, 'String');
udl = str2double(udl);
if isnumeric(udl)
    if udl > 0
        UDL = udl;
        fprintf('UDL set to: %d [N/m].\n',UDL);
    else
        fprintf('UDL must be a positive number.\n');
    end
else
    fprintf('UDL must be a positive number.\n');
end
...
%% WIND SETUP
function pb_windConfig(varargin)
        % Get structure from the main function. 
    S = varargin{3}; 

    % Create new figure
    T.fh = figure('units','pixels',...
                  'position',[440 210 220 180],...
                  'menubar','none',...
                  'numbertitle','off',...
                  'name','Wind configuration',...
                  'resize','off');
              
        %Create width setup
    T.windpb = uicontrol('style','push',...
                      'unit','pix',...
                      'position',[6 95 195 30],...
                      'string','Set wind velocity');
    T.winded1 = uicontrol('style','edit',...
                         'unit','pix',...
                         'position',[6 140 101 30],...
                         'string','Wind Speed [km/h]:');
    T.winded2 = uicontrol('style','edit',...
                         'unit','pix',...
                         'position',[111 140 99 30],...
                         'string','Positive number');
                 
    % Make sure the edit box cannot be altered.                 
    set(T.winded1,'enable','off');
    % Set callback when the button is pushed.
    set(T.windpb,'call',{@wind_callback,T});
    
    T.exit = uicontrol('style','push',...
                   'units','pix',...
                   'position',[10 20 183 50],...
                   'string','Return to main menu',...
                   'callback',{@pb_close,T});    
    % When the main function is closed, this GUI will close too.               
    set(S.fh,'deletefcn',{@fig_delet,T.fh})
...
function wind_callback(varargin)
    % Load beam width.
global WindSpeed
...
% Import beam width string from the edit box. Then change it to number.
windSpeed = get(T.winded2, 'String');
windSpeed = str2double(windSpeed);
if isnumeric(windSpeed)
    if windSpeed > 0
        WindSpeed = windSpeed;
        fprintf('WindSpeed set to: %d [km/h].\n',WindSpeed);
    else
        fprintf('WindSpeed must be a positive number.\n');
    end
else
    fprintf('WindSpeed must be a positive number.\n');
end
...
%% BUOYANCY SETUP
function pb_buoyancyConfig(varargin)
            % Get structure from the main function. 
    S = varargin{3}; 

    % Create new figure
    T.fh = figure('units','pixels',...
                  'position',[440 210 250 160],...
                  'menubar','none',...
                  'numbertitle','off',...
                  'name','Beam width, height, type configuration',...
                  'resize','off');
                    ...
                    %Create beam type setup
    T.fluidTypepb = uicontrol('style','push',...
                      'unit','pix',...
                      'position',[6 85 195 30],...
                      'string','Set fluid');
    T.fluidTypeed1 = uicontrol('style','edit',...
                         'unit','pix',...
                         'position',[6 120 131 30],...
                         'string','Set Fluid:');
    T.fluidTypeed2 = uicontrol('style','edit',...
                         'unit','pix',...
                         'position',[141 120 99 30],...
                         'string','Honey Water');
                 
    % Make sure the edit box cannot be altered.                 
    set(T.fluidTypeed1,'enable','off');
    % Set callback when the button is pushed.
    set(T.fluidTypepb,'call',{@fluidType_callback,T});
...
function fluidType_callback(varargin)
    % Load beam width.
    global FluidType
    ...
    % Import beam width string from the edit box. Then change it to number.
    fluidType = get(T.fluidTypeed2, 'String');
    fluidType = lower(fluidType);
    if strcmp(fluidType,'honey')
        FluidType = 'honey';
        fprintf('FluidType set to: %s.\n',FluidType);
    elseif strcmp(fluidType,'water')
        FluidType = 'water';
        fprintf('FluidType set to: %s.\n',FluidType);
    else
        fprintf('Wrong fluid type.Please choose water/honey.\n');
    end
end
%------------------------------------------------------------------
%% CALCULATE DEFLECTION
...
function pb_plotDeflectionDiagram(varargin)
    global bgselect flMagnitudes beamlength beamWidth beamHeight beamMaterial ElasticMod Inertia UDL WindSpeed FluidType
    plotDeflection(bgselect,beamlength,beamWidth,beamHeight,beamMaterial,ElasticMod,Inertia,str2double(flMagnitudes(2)),UDL,WindSpeed,FluidType)
end
%-------------------------------------------------------------------------
%% END BEAM SETUP
...
function pb_close_main(varargin)
close(gcbf)  
end
